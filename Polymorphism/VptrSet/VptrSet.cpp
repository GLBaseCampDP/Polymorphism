...
#define MEMORY(p, cnt, m) { int *p1 = (int*)p;\
  cout << "\nstart adress " << m << ": " << (long long)p << "\n";\
  for (int i = 0; i < cnt; ++i) \
	cout << "    ofsset - " << i << " sizeof (int) - " << *(p1 + i) << "\n"; }

#define VPTRVTAB(p) { long long *p2 = (long long*)p;\
...
struct P : B, D {
	CLASS("P");
	P(int j0, int j1, int j2, int j3, int j4) : B(j1, j2), D(j3, j4), i(j0) {}
};


#pragma pack(pop)
void memory(A* p, int cnt, char* m) MEMORY(p, cnt, m);

void memory(C* p, int cnt, char* m) MEMORY(p, cnt, m);
